  ...
  return (
    <div
      className={`relative flex flex-col gap-4 rounded-lg p-6 transition-shadow hover:shadow-xl ${
        isPopular ? 'border-2 border-blue-600 bg-white shadow-2xl' : 'border border-slate-200 bg-slate-50'
      }`}
    >
      {isPopular && (
      ...
      <div className="flex flex-col gap-1">
        <h3 className={`text-lg font-semibold ${titleClass || 'text-slate-800'}`}>{title}</h3>
      ...
      <button
        ...
        } ${buttonHoverClass || 'hover:bg-slate-700'} ${borderClass || 'focus:ring-slate-500'}`}
      >
        {buttonText}
      </button>
