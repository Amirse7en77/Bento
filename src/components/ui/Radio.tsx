...
const Radio: React.FC<RadioProps> = ({ label, name, id, ...props }) => {
  const uniqueId =
    id ||
    `radio-${name}-${label.replace(/\s+/g, "-")}-${Math.random()
      .toString(36)
      .substr(2, 9)}`;
  ...
  return (
    <label
      htmlFor={uniqueId}
      className="flex items-center gap-3 rounded-lg border border-solid border-[#cedbe8] p-3 cursor-pointer hover:border-[#0c7ff2] transition-colors"
    >
      <input
        id={uniqueId}
        type="radio"
